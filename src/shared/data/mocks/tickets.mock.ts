...
export const getTicketStats = () => {
  return {
    ...
    open: mockTickets.filter((t) => t.status === "Yeni İstek").length,
    inProgress: mockTickets.filter((t) => t.status === "Atanan").length,
    resolved: mockTickets.filter((t) => t.status === "Çözümlenen").length,
